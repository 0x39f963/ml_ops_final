...
mdd (metrics-driven development) - это когда архитектурное решение принимают не на глаз, а по статистике на реальных замерах.

проверял такую идею: если тяжелый расчет признаков вынести заранее в пакетную обработку и кэш, а не считать его в момент запроса, станет ли сервис отвечать заметно быстрее. для этого сравнил два набора замеров времени ответа: старая схема (считаем прямо в запросе) и улучшенная (берем готовое из кэша).
...
- **решение (зафиксировано в adr).** тяжелый расчет признаков выносим в пакетную обработку + кэш (feature store), а api отдает уже посчитанный топ-10. так держим время ответа `/score` в пределах p95 < 500 мс (то есть 95% запросов укладываются в полсекунды).

adr (architecture decision record) - это короткий документ, где зафиксировано принятое архитектурное решение и почему.
