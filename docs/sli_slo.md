...
Слежу за здоровьем сервиса по набору показателей (SLI) и держу для каждого границы нормы, тревоги и аварии (SLO). Если показатель уходит за границу, на это есть заранее описанное действие.

Показатели на трех уровнях:
- технический - быстро ли и без ошибок отвечает сервис;
- модель и данные - не деградирует ли качество модели и не испортились ли входные данные;
- бизнес - приносит ли система пользу продажам.

Числовые пороги ниже - предварительные: окончательно откалибрую их после первого боевого прогона и после того, как накопится факт следующего квартала. Точные формулы для Prometheus - в приложении в конце.
...
- поэтому числа выше - рабочая отправная точка; финальные SLA установим после калибровки на боевом прогоне, структуру документа при этом не меняем.
